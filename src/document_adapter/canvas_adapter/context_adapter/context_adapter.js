...
class ContextAdapter{
  ...
  scale(x,y){
    ...
    var mult = math.matrix([[x,0,0],[0,y,0],[0,0,1]]);
  ...
  translate(x,y){
    ...
    var mult = math.matrix([[1,0,x],[0,1,y],[0,0,1]]);
    this.transformMatrix = math.multiply(this.transformMatrix,mult);
    console.log("transformMatrix: "+ this.transformMatrix);
  }
  ...
  drawImage(image){
    ...
    if(this.jsCanvas){
      ...
      this.jsCanvas.addImage(this.nodeContext.canvas.toDataURL("image/png"),x,y,width,height);
    }
    return ret;
  }
